...
export interface IVertexOptions extends fabric.IGroupOptions{
	identity?: string;
	vertex?: Vertex;
...
export class Vertex extends fabric.Group{

	static circleRadius:number=15;

	private attachments: fabric.Object[];
	private attachmentSubs: Map<fabric.Object, Subscription[]> ;

	private _text:fabric.Text;
	private _circle:fabric.Circle;

	private _tempLine: Connector;
	private _tempVertex: Vertex;
	...
	constructor(options: IVertexOptions) {
		...
		let text = new fabric.Text(options.identity!, {
			originX:'center',
			originY:'center',
			fontSize:20,
			textAlign:'center',
		...
		this._position = new BehaviorSubject<fabric.Point>(new fabric.Point(this.left!,this.top!));
		...
		this.attachments = [];
		this.attachmentSubs = new Map<fabric.Object, Subscription[]>();
	...
	mouseDown(ev:any){
		...
		if (settingsService.isMoveable) {
			...
			this.canvas?.on('mouse:up', this._mouseUpEventRef);
			...
			if (settingsService.currentBondType == BondType.lonePair){
				...
				this._tempLine = new Connector([center.x, center.y, center.x + 50, center.y],{
					vertex1: this,
